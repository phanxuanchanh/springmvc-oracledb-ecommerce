...
public class InvoiceStateDAO {
	...
	public List<InvoiceState> GetInvoiceStates(){
		...
		String query = "Select * from ThuongMaiDienTu.InvoiceStates";
	...
	public InvoiceState GetInvoiceState(BigDecimal id) {
		...
		String query = "Select * from ThuongMaiDienTu.InvoiceStates where ID = ?";
	...
	public boolean CreateInvoiceState(InvoiceState invoiceState) {
		String query = "Insert into ThuongMaiDienTu.InvoiceStates(\"name\", \"description\", createAt, updateAt) values(?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
	...
	public boolean UpdateInvoiceState(InvoiceState invoiceState) {
		String query = "Update ThuongMaiDienTu.InvoiceStates set \"name\" = ?, \"description\" = ?, updateAt = CURRENT_TIMESTAMP where ID = ?";
	...
	public boolean DeleteInvoiceState(BigDecimal id) {
		String query = "Delete from ThuongMaiDienTu.InvoiceStates where ID = ?";
		int affected = jdbcTemplate.update(query, new Object[] { id });
		return (affected > 0);
	}
	
	public int CountInvoiceState() {
		String query = "Select count(*) from ThuongMaiDienTu.InvoiceStates";
		int count = jdbcTemplate.queryForObject(query, Integer.class);
		return count;
	}
	
	public boolean IsExistInvoiceStateById(BigDecimal id) {
		String query = "Select count(*) from ThuongMaiDienTu.InvoiceStates where ID = ?";
	...
	public boolean IsExistInvoiceStateByName(String name) {
		String query = "Select count(*) from ThuongMaiDienTu.InvoiceStates where \"name\" = ?";
