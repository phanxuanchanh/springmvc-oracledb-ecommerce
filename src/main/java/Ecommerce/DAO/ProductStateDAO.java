...
public class ProductStateDAO {
	...
	public List<ProductState> GetProductStates(){
		...
		String query = "Select * from ThuongMaiDienTu.ProductStates";
	...
	public ProductState GetProductState(BigDecimal id) {
		...
		String query = "Select * from ThuongMaiDienTu.ProductStates where ID = ?";
	...
	public boolean CreateProductState(ProductState productState) {
		String query = "Insert into ThuongMaiDienTu.ProductStates(\"name\", \"description\", createAt, updateAt) values(?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
	...
	public boolean UpdateProductState(ProductState productState) {
		String query = "Update ThuongMaiDienTu.ProductStates set \"name\" = ?, \"description\" = ?, updateAt = CURRENT_TIMESTAMP where ID = ?";
	...
	public boolean DeleteProductState(BigDecimal id) {
		String query = "Delete from ThuongMaiDienTu.ProductStates where ID = ?";
		int affected = jdbcTemplate.update(query, new Object[] { id });
		return (affected > 0);
	}
	
	public int CountProductState() {
		String query = "Select count(*) from ThuongMaiDienTu.ProductStates";
		int count = jdbcTemplate.queryForObject(query, Integer.class);
		return count;
	}
	
	public boolean IsExistProductStateById(BigDecimal id) {
		String query = "Select count(*) from ThuongMaiDienTu.ProductStates where ID = ?";
	...
	public boolean IsExistProductStateByName(String name) {
		String query = "Select count(*) from ThuongMaiDienTu.ProductStates where \"name\" = ?";
